...
// missile relocation and damage must be computed before moving robots

        private double _double_range;
        ...
        public double _double_explodingTime;

        public void Update(double dt)
        {
            double travelledDistance = dt * ParametersSingleton.MissileSpeed; // distance increase due to speed d = v.t
            if (_double_range > travelledDistance) // target not reached
            {
                _double_range -= travelledDistance; // update remaining distance to travel
                _double_locX += travelledDistance*_double_cos_driveAngle; // relocation along x axis
                _double_locY += travelledDistance*_double_sin_driveAngle; // relocation along y axis
            }
            else // target reached, explode
            {
                _explosionTick = Tick.Now;
                _double_explodingTime = _double_range / ParametersSingleton.MissileSpeed; // exploding time
                _double_locX += _double_range * _double_cos_driveAngle; // relocation along x axis
                _double_locY += _double_range * _double_sin_driveAngle; // relocation along y axis
                State = MissileStates.Exploding;
            }
