...
namespace Arena.Internal
{
    ...
    internal class PreciseArena : IReadonlyArena
    {
        private struct DamageByRange
        {
            public int Range;
        ...
        private readonly List<PreciseRobot> _robots;
        private readonly List<PreciseMissile> _missiles;
        private int _missileId;
        ...
        internal PreciseArena()
        {
            ...
            //
            _robots = new List<PreciseRobot>();
            _missiles = new List<PreciseMissile>();
        ...
        public void CHEAT_FindNearestEnemy(PreciseRobot robot, out double degrees, out double range, out double x, out double y)
        {
            degrees = 0;
            range = 0;
            x = 0;
            ...
            double nearest = Double.MaxValue;
            foreach (PreciseRobot otherRobot in _robots.Where(r => r.Team != robot.Team))
            {
                double distance = Common.Math.Distance(robot.LocX, robot.LocY, otherRobot.LocX, otherRobot.LocY);
                if (distance < nearest)
                {
        ...
        public void CHEAT_FireAt(PreciseRobot robot, double targetX, double targetY)
        {
            ...
            lock (_missiles)
            {
                PreciseMissile missile = new PreciseMissile(robot, MatchStart, _missileId, robot.LocX, robot.LocY, heading, range);
                _missiles.Add(missile);
                _missileId++;
            }
        }

        public int Cannon(PreciseRobot robot, Tick launchTick, int degrees, int range)
        {
            //Log.WriteLine(Log.LogLevels.Debug, "Launching missile from Robot {0}[{1}] to {2} {3}", robot.TeamName, robot.Id, degrees, range);
            lock (_missiles)
            {
                PreciseMissile missile = new PreciseMissile(robot, MatchStart, _missileId, robot.LocX, robot.LocY, degrees, range);
                _missiles.Add(missile);
                _missileId++;
            }
            return 1;
        ...
        public void Drive(PreciseRobot robot, int degrees, int speed)
        {
            // NOP: managed in InternalRobot
        }

        public int Scan(PreciseRobot robot, int degrees, int resolution)
        {
            double nearest = Double.MaxValue;
            PreciseRobot target = null;
            foreach (PreciseRobot r in _robots.Where(x => x != robot && x.State == RobotStates.Running && x.Damage < ParametersSingleton.MaxDamage))
            {
                // Sector method
                bool isInSector = Common.Math.IsInSector(robot.LocX, robot.LocY, degrees, resolution, r.LocX, r.LocY);
                if (isInSector)
        ...
        public int TeamCount(PreciseRobot robot)
        {
            return _robots.Count(x => x.Team == robot.Team);
        }
        ...
        private void InitializeMatch(int count, Func<int, dynamic> getCoordinatesFunc, params Type[] teamType)
        {
                ...
                if (State != ArenaStates.Error)
                {
                    ...
                    for (int i = 0; i < count; i++)
                        for (int t = 0; t < teamType.Length; t++)
                        {
                            ...
                            int y = getCoordinatesFunc(t).Y;
                            SDK.Robot userRobot = Activator.CreateInstance(teamType[t]) as SDK.Robot;
                            PreciseRobot robot = new PreciseRobot();
                            robot.Initialize(userRobot, this, teamType[t].Name, i, t, x, y);
                            _robots.Add(robot);
            ...
            catch (Exception ex)
            {
                ...
                // Force robots to stop
                foreach (PreciseRobot robot in _robots)
                    robot.Stop();
        ...
        private void Step()
        {
            if (State == ArenaStates.Running)
            {
                ...
                else
                {
                    ...
                    lock (_missiles)
                    {
                        foreach (PreciseMissile missile in _missiles)
                        {
                            if (missile.State == MissileStates.Flying)
                            {
                                ...
                                if (missile.State == MissileStates.Exploding)
                                {
                                    PreciseRobot missileRobot = missile.Robot as PreciseRobot;
                                    foreach (PreciseRobot robot in _robots.Where(x => x.State == RobotStates.Running))
                                    {
